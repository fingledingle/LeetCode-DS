...
function find_length(stringGuy){ //we make the function and pass the string or the array
    ...
    for(let right = 0; right < stringGuy.length; i++){  //making the loop so the right gets bigger
        if (string[right] == 0){ //if the current guy is 0 then we add 1 to the current 
            curr ++;
        }
...
console.log(find_length([1101100111]))
